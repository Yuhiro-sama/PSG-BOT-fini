...
class Purchase(commands.Cog):
    ...
    async def addcoin(self, ctx, user: discord.Member, coin:int):
        update_user(user, "coin", coin)
        await ctx.send("Réussit ! ")
        
        
    @commands.command()
    @commands.has_guild_permissions(administrator=True)
    async def removecoin(self, ctx, user: discord.Member, coin:int):
        update_user(user, "coin", -coin)
        await ctx.send("Réussit ! ")
    ...
    async def addshop(self, ctx, role : discord.Role, coins :int):
    ...
    async def buy(self, ctx,*, role: discord.Role=None):
        ...
        await update_user(ctx.author, "coin", -price)
